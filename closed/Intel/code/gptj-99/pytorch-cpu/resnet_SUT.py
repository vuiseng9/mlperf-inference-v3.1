...
class Consumer(mp.Process):
    ...
    def doWarmup(self):
        ...
        input_data = warmup_data.repeat(self.batch_size, 1, 1, 1)
    ...
    def handleTasks(self, i, task_queue, result_queue, pid, start_core, num_cores):
        ...
        while True:
            try:
                ...
                feature_map = self.data_obj.getSamples(sample_index_list)
                output = self.model.predict(feature_map)
                result = self.data_obj.postProcess(query_id_list, sample_index_list, output)
    ...
    def run(self):
        ...
        from resnet_backend import ResnetBackend as Backend
        ...
        from imagenet_dataset import Dataset
        self.data_obj = Dataset(self.dataset_path, self.total_sample_count)
