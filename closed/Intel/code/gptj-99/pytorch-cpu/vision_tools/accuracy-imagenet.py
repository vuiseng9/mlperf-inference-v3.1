...
import numpy as np
from dataset_utils import create_imagenet_valset_loader
...
def get_args():
    ...
    parser.add_argument("--dtype", default="int64", choices=["float32", "int32", "int64"], help="data type of the label")
...
def main():
    ...
    imagenet = []
    # get mapping ground truth
    val_loader = create_imagenet_valset_loader(args.dataset_path, batch_size=10)
    cnt=0
    for i, (input_, target) in enumerate(val_loader):
        for t in target.numpy():
            imagenet.append(int(t))
            cnt+=1
        if cnt == args.total_sample_count:
            break
